...
export default class LabelForm extends React.Component<Props> {
    ...
    private renderAttributeNameInput(fieldInstance: any, attr: any): JSX.Element {
        ...
        return (
            ...
            >
                <Input disabled={attr.id >= 0} className='cvat-attribute-name-input' placeholder='名称' />
            </Form.Item>
        );
    }
    ...
    private renderAttributeValuesInput(fieldInstance: any, attr: any): JSX.Element {
        ...
        return (
            <CVATTooltip title='按 Enter 键添加新值'>
                <Form.Item
                    name={[key, 'values']}
                    rules={[
                        {
                    ...
                    <Select
                        ...
                        tagRender={(props) => {
                            ...
                            return (
                                <CVATTooltip
                                    placement='bottom'
                                    title={isDefault ? '该值是默认值' : '点击设置为默认值'}
                                >
                                    <Tag
                                        visible
                                        onMouseEnter={() => {
    ...
    private renderNumberRangeInput(fieldInstance: any, attr: any): JSX.Element {
        ...
        const validator = (_: any, strNumbers: string): Promise<void> => {
            ...
            if (numbers.length !== 3) {
                return Promise.reject(new Error('预计三个数字'));
            }
            ...
            if (max - min < step) {
                return Promise.reject(new Error('步骤必须小于最小最大差异'));
            }

            if (step <= 0) {
                return Promise.reject(new Error('步骤必须是正数'));
            }
                ...
                <Form.Item>
                    <Button
                        ...
                        onClick={(): void => {
                            if (attr.id >= 0) {
                                Modal.confirm({
                                    title: `是否确认删除名为 "${attr.name}" 的属性?`,
                                    content: '此操作是不可逆转的。 它将删除相应的注释',
                                    onOk: () => {
                    ...
                    },
                    {
                        validator: (_rule: any, labelName: string) => {
                            if (labelNames.includes(labelName) && label?.name !== labelName) {
                                return Promise.reject(new Error('标签名称对于任务必须是唯一的'));
                            }
                            return Promise.resolve();
                        },
                    },
